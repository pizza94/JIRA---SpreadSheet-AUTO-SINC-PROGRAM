...
import { extractIssueKeys } from "../scripts/issue-key-parser.mjs";
...
const jiraBaseUrl =
  process.env.JIRA_BASE_URL ?? "http://192.168.0.119:8079";
...
async function createMainWindow() {
  mainWindow = new BrowserWindow({
    ...
    // 무설치 EXE의 압축 해제 이후에는 창을 바로 표시해 로딩 대기 체감을 줄인다.
    show: true,
...
async function writeFailureResult(
  ...
  const failureLogPath = path.join(
    targetDirectory,
    `failure-${type}-${failureTime.toISOString().replace(/[:.]/g, "-")}.log`
  );
  await writeFile(
    failureLogPath,
    [
      `[Failure job] ${type}`,
      `[Failure time] ${formatKoreanDateTime(failureTime)}`,
      "",
      "[stdout]",
      output || "-",
      "",
  ...
  const failureDetails = formatFailureDetails(errorOutput, output);
  const issueKeys = [
    ...new Set(
  ...
  const content = [
    ...
    "",
    "[Failure details]",
    ...(failureDetails.length ? failureDetails : ["- Check the raw failure log."]),
    "",
    `[Raw failure log] ${failureLogPath}`
  ].join("\r\n");
  return prependDailyResult(targetDirectory, content, failureTime);
}
...
function parseIssueKeys(value) {
  const keys = extractIssueKeys(value);
  if (!keys.length) {
...
function summarizeError(text) {
  ...
  const meaningfulLines = lines.filter(
    (line) =>
      ...
      !/^at\s+/i.test(line) &&
      !/^file:\/\//i.test(line) &&
      !/\bthrow\s+new\s+Error\s*\(/i.test(line) &&
      !/^\^+$/.test(line) &&
      !/^[{}\[\],]+$/.test(line) &&
...
function formatFailureDetails(errorOutput, output) {
...
async function clearPrivateState() {
  ...
  try {
    ...
    outputDirectory = validateOutputDirectory(stored.outputDirectory);
  } catch {
    // 설정이 없거나 손상된 경우에는 실행한 사용자 바탕화면을 기본값으로 사용한다.
  }
  await Promise.allSettled([
    rm(jiraAuthPath(), { force: true }),
    rm(googleAuthPath(), { force: true })
  ]);
  // 종료 시 Jira 세션과 이슈/시트 입력값은 초기화하되, 사용자가 선택한 결과 폴더는 유지한다.
  await saveSettings({
    ...createDefaultSettings(),
    outputDirectory
  });
