...
export function findTemplateTypeStyleSource(rows, layout, issueType) {
  const expected = normalizeIssueTypeForStyle(issueType);
  if (!expected) {
    return null;
  }

  for (let index = layout.headerIndex + 1; index < layout.issueEndIndex; index += 1) {
    if (normalizeIssueTypeForStyle(rows[index]?.[1]) === expected) {
      return { rowIndex: index, columnIndex: 1, source: "issue-row" };
    }
  }

  // 신규 유형은 기준 시트의 이슈 행이 없을 수 있다. 이때 상단 건수 합계표의
  // B열 유형 셀을 색상 기준으로 사용한다.
  for (let rowIndex = 0; rowIndex < layout.headerIndex; rowIndex += 1) {
    if (normalizeIssueTypeForStyle(rows[rowIndex]?.[1]) === expected) {
      return { rowIndex, columnIndex: 1, source: "summary" };
    }
  }
  return null;
}
