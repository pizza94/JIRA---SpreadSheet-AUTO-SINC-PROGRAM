...
const JIRA_STATUS_CONTROLLED_VALUES = new Set([
  "테스트중",
  "개발중",
  "재개발",
  "재개발요청",
...
const JIRA_DEVELOPMENT_IN_PROGRESS_STATUSES = new Set([
  "작업진행중",
  "개발진행중",
  "개발예정",
  "재개발요청",
...
export function shouldUseJiraStatusForSnapshot(
  previousValue,
  includedInInput
) {
  return (
    includedInInput === true &&
    JIRA_STATUS_CONTROLLED_VALUES.has(normalizeSnapshotValue(previousValue))
  );
}

export function snapshotValueForRun(
  previousValue,
  includedInInput,
  jiraStatus = ""
) {
  if (shouldUseJiraStatusForSnapshot(previousValue, includedInInput)) {
    const normalizedStatus = normalizeSnapshotValue(jiraStatus);
    if (JIRA_DEVELOPMENT_IN_PROGRESS_STATUSES.has(normalizedStatus)) {
      return "개발중";
    }
    return JIRA_STATUS_SNAPSHOT_VALUES.get(normalizedStatus) ?? "";
  }
  return carriedSnapshotValue(previousValue);
}
...
export function snapshotCarryAction(
  ...
  const value = snapshotValueForRun(
    previousValue,
    includedInInput,
    jiraStatus
  ).replace(
